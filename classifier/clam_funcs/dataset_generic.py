...
from clam_funcs.utils.utils import generate_split, nth
...
class Generic_WSI_Classification_Dataset(Dataset):
	def __init__(self,
		...
		self.data_dir = None
		self.label_col = label_col if label_col else 'label'
		...
		# print(self.slide_data.head(5))
		# self.slide_data=self.slide_data[self.slide_data['ihc_block']]
		# self.slide_data=self.slide_data.reset_index()
		# print(self.slide_data.head(5))
	...
	def patient_data_prep(self, patient_voting='max'):
		...
		for p in patients:
			...
			if patient_voting == 'max':
				...
				label = label[0] if len(label==1) else stats.mode(label)[0]
			else:
	...
	def summarize(self):
		print("Training Augmentations: ", self.augments)
		print("Training Patch Fraction", self.patch_frac)
	...
	def return_splits(self, csv_path=None):
		...
		return train_split, val_split, val_split#test_split
...
class Generic_Split(Generic_MIL_Dataset):
	def __init__(self, slide_data, data_dir=None, num_classes=2,augments=['og'],patch_frac=1.0):
		...
		self.augments=augments
		self.patch_frac=patch_frac
		print(f"Split Patch Fraction {patch_frac}")
		print(f"Split Augments {self.augments}")
...
def get_feats(data_dir,aug,slide_id,rng,patch_frac):
	...
	try:
		...
		#return torch.ones((2,1024)),-1,np.ones((2,2))
		raise FileExistsError(f"file {full_path} likely doesnt exist")
	if(patch_frac<1.0):
		n_patch = features.shape[0]
		subset_indices=np.sort(rng.choice(n_patch,int(n_patch*patch_frac),replace=False))

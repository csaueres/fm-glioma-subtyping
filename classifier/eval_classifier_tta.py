...
def get_results_for_fold(model, eval_data, f, args):
    ...
    for d in eval_data:
        d.load_from_h5(True)
        d_loader = get_split_loader(d,training=False)
        loaders.append(d_loader)
    results_dict,acc_logger ,(val_error, val_auc, val_f1, val_mcc) = summary(model, loaders, args.n_class)

    return acc_logger, 1-val_error, val_auc, val_f1, val_mcc



def summary(model, loaders, n_class):
...
if __name__ == "__main__":
    ...
    for f in range(args.k):
        acc_logger, acc, auc, f1, mcc = get_results_for_fold(model,datasets,f,args)
        class_accs=[]
    ...
    row_names = ["1","2","3","4","5","Avg","StdDev"]
